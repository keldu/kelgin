...
public:
	FreeTypeFont(Our<FreeTypeContext> ctx, FT_Face face)
		: context{std::move(ctx)}, face{face} {}
	~FreeTypeFont() {
		assert(face);
		if (face) {
			FT_Done_Face(face);
	...
	Our<Page> generatePage(uint32_t size,
						   const std::set<uint32_t> &code_points) override {
		return nullptr;
	}
};
